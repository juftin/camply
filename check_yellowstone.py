...
"""
Check Xanterra API for Yellowstone Campground Booking Availability
"""
...
class YellowstoneLodging(object):
    ...
    def check_yellowstone_lodging(cls, booking_start: datetime, number_of_nights: int = 1,
        ...
        """
        xanterra_date_formatting = booking_start.strftime("%m/%d/%Y")
        query_dict = dict(date=xanterra_date_formatting,
                          nights=number_of_nights,
                          limit=number_of_nights,
                          adults=number_of_guests,
                          rate_code="INTERNET")
        api_endpoint = cls._get_api_endpoint(query=query_dict)
        logger.info(f"Searching for Yellowstone Lodging Availability: {xanterra_date_formatting}")
        ...
        try:
            ...
            error_message = "Something went wrong with checking the Xanterra API"
...
if __name__ == "__main__":
    ...
    YellowstoneLodging.continuously_check_for_availability(
        booking_start=datetime.strptime(environ["BOOKING_DATE_START"],
                                        "%Y-%m-%d"),
